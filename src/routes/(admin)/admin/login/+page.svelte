<script lang="ts">
  ...
  const handleLogin = () => {
    ...
    if (email == null || !email?.includes('@mobilegreaser.com')) {
      errorMessage = 'Invalid admin email provided.';
    }
  ...
  const validateEmailField: EventHandler = (e: Event) => {
    const { target } = e;
  ...
  <div class="right w-50">
    <div class="w-75 mx-auto h-100 d-flex flex-column justify-content-center align-items-center">
      <header class="mb-1">
        ...
        <p class="text-secondary text-center">Enter your email to sign in</p>
      </header>

      <form action="" class="w-100 p-3">
        <div class="p-2 mb-3">
          <input type="text" on:input={validateEmailField} placeholder="Email" class="form-control" bind:value={email}>
          {#if errorMessage}
