<script lang="ts">
  ...
  const getVehicles = async (pg: number, size: number, year: string | null, make: string | null, model: string | null, trim: string | null) => {
    const vehiclesResp = await fetch(`/api/vehicle?year=${year ?? ''}&make=${make ?? ''}&model=${model ?? ''}&trim=${trim ?? ''}&page=${pg}&size=${size}`, {
      method: 'GET',
      headers: {
...
<main class="container mx-auto mt-4">
  ...
  {/if}
  <div class="card card-normal bg-white p-2">
    ...
    <div>
      ...
      <select class="select select-bordered w-full max-w-xs" bind:value={make} disabled={year == null}>
      ...
      <select class="select select-bordered w-full max-w-xs" bind:value={model} disabled={make == null}>
...
<!--      </select>-->
    ...
    <div class="join p-2 flex justify-center">
      <button class="join-item btn" on:click={() => page = page > 1 ? page - 1 : page}>«</button>
      <button class="join-item btn">Page {page}</button>
      <button class="join-item btn" on:click={async () => page = size <= (await vehicles)?.length ? page + 1 : page}>»</button>
    </div>
    {:catch error}
      <p>{error.message}</p>
    {/await}
